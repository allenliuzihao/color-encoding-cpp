...
namespace UnitTests
{
    ...
    public:
        TEST_METHOD(FP16Specials)
        {
            ...
            assert(parsedFloat.sign == 1 && parsedFloat.exponent == 30 && parsedFloat.mantissa == 1023 && value == -65504);

            parsedFloat = Float16(float(65520));
            value = parsedFloat.value();
            assert(parsedFloat.sign == 0 && parsedFloat.exponent == 31 && parsedFloat.mantissa == 0 && std::isinf(value));
            ...
            parsedFloat = Float16(float(-65521));
            value = parsedFloat.value();
            assert(parsedFloat.sign == 1 && parsedFloat.exponent == 31 && parsedFloat.mantissa == 0 && std::isinf(value) && value < 0);
            
            parsedFloat = Float16(float(125521));
            value = parsedFloat.value();
            assert(parsedFloat.sign == 0 && parsedFloat.exponent == 31 && parsedFloat.mantissa == 0 && std::isinf(value) && value > 0);

            parsedFloat = Float16(asfloat(0x7f800001));
            value = parsedFloat.value();
            assert(parsedFloat.sign == 0 && parsedFloat.exponent == 31 && parsedFloat.mantissa == 1 && std::isnan(value));
        }
